...
public class TraceRequestWrapper extends HttpServletRequestWrapper {
	...
	public String toStringWithBody() {
		...
		boolean forceBinaryPrint = ConfigurationFactory.getConfiguration().getBoolean("service." + serviceName + ".http.traceFilter.printInHex", false);
		String contentType = getContentType();
		if (contentType == null) {
			if (getRequest() instanceof Request) {
				Request request = (Request) getRequest();
				contentType = request.getHeader(HttpHeader.CONTENT_TYPE.toString());
			}
		}
	...
	private class ServletInputStreamImpl extends InputStream {
